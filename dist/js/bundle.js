...
function slider() {
  var slideIndex = 1,
      // это перемення отвечает за слайд который находится в текущий момент
  slides = document.querySelectorAll('.slider-item'),
      //взыввем наши слайдеры все
  prev = document.querySelector('.prev'),
      //взыввем наши // cтрелочку крторая двигает слайдер назад
  next = document.querySelector('.next'),
      //взыввем наши // cтрелочку крторая двигает слайдер вперед
  dotsWrap = document.querySelector('.slider-dots'),
      //получаем обертку наших точек
  dots = document.querySelectorAll('.dot'); //получаем все точки которые будут использоваться

  showSlides(slideIndex);

  function showSlides(n) {
    //это функция будет принимать один аргумент для того чобы она переключала слайды
    if (n > slides.length) {
      // пишим условие для слайдов
      slideIndex = 1; //установливвем слайдер на 1
    }

    if (n < 1) {
      // пишим обратное условие
      slideIndex = slides.length; // если мы нажимаем стрелку назад на 1 слайде показывается последний слайд
    }

    slides.forEach(function (item) {
      return item.style.display = 'none';
    }); //1 способ каждому слайдеру передаём убрать дисплей
    ...
    }); // уберает классы dot-active  с точек
  ...
  function plusSlides(n) {
    //это функция меняет slideIndex для чтобы менять картинки в слайде
    showSlides(slideIndex += n);
  }

  function currentSlaide(n) {
    //это функция прописывается чтобы узнать какой сейчас слайд стоит
    showSlides(slideIndex = n);
  }

  prev.addEventListener('click', function () {
    //это пишется для кнопки перемещение назад
    plusSlides(-1);
  });
  next.addEventListener('click', function () {
    //это пишется для кнопки перемещение вперед
    plusSlides(1);
  });
  dotsWrap.addEventListener('click', function (event) {
    //эту функция для точек  в слайдах
